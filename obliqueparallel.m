...
f = 10e6;% frequency
thetaI = 0:1:89; %theta incident varies between 0 and 90
%Copper 
uRCopp = 1; 
...
TransCopp = abs((2*ettaCopp*cosd(thetaI))/(ettaSpace*cosd(thetaI)+ettaCopp*cosd(thetaTCopp))); %transmission coefficient for copper
attenCopp = (pi*f*u*uRCopp*sigCopp)^0.5; %attenuation for copper
thickCopp = ((log(1e-6/TransCopp))/(-attenCopp))*cosd(thetaTCopp); %thickness for copper
%aluminum
sigAl = 3.538*1e7;
uRAl = 1;
thetaTAl = asind(sind(thetaI)/(uRAl)^0.5);
ettaAl = abs((1 + j)*(((2*pi*f*u*uRAl)/(2*sigAl))^0.5));
TransAl = abs((2*ettaAl*cosd(thetaI))/(ettaSpace*cosd(thetaI)+ettaAl*cosd(thetaTAl)))
...
TransMu = abs((2*ettaMu*cosd(thetaI))/(ettaSpace*cosd(thetaI)+ettaMu*cosd(thetaTMu)));
...
TransInc = abs((2*ettaInc*cosd(thetaI))/(ettaSpace*cosd(thetaI)+ettaInc*cosd(thetaTinc)));
...
%exporting
csvwrite('resultsparallel.csv',thickvec);